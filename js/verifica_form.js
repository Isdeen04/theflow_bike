...
/*
---------------------------------------------------------------------
----- VALIDATORE GENERICO DI FORM (v. 1.0.2) 01 marzo 2024 ----------x
----- www.androgino.it ----------------------------------------------
---------------------------------------------------------------------
...
function ControllaMail(frmMail,NCampo){
	...
	if (EmailAddr!=""){		
		...
		if (Filtro.test(EmailAddr)) return true;
			...
			alert(TXT_ATTENZIONE+" "+TXT_erroreFormatoEmail.replace("[1]"," '"+NCampo+"' "));
			frmMail.focus();
			return false;
		}
	}else return true;
...
function ControllaNumero(frmNum,NCampo,nt){
	...
	if (frmNum.value.length>0){		
		if(!nt){
			...
			Filtro = /[\D]+/;
			add=" "+TXT_numeriInteri;
		}
		if (!Filtro.test(NumVal)) return true;
		else{
			alert(TXT_ATTENZIONE+" "+TXT_campoNumerici.replace("[1]"," '"+NCampo+"' ")+add);
			frmNum.focus();
			return false;
		}
	}else return true;
...
function verifica_form(frm){
	for (k=0; k<frm.elements.length; k++){
		...
		ver_value=trim(frm.elements[k].value);
		...
		if (ver_contr){
			if (ver_type=="text" || ver_type=="email" || ver_type=="textarea" || ver_type=="hidden" || ver_type=="password"){ // se è un testo
				if(ver_value.length<ver_lMin){
					if(ver_lMin>1)alert(TXT_lunghezzaMinCampo.replace("[1]","'"+ver_etichetta+"' ").replace("[2]"," "+ver_lMin).replace("[3]"," "+ver_value.length));
					else alert(TXT_inserireValore.replace("[1]"," '"+ver_etichetta+"' "));
					if (ver_type!="hidden")frm.elements[k].focus();
					return false;
				}
				if(ver_value.length>ver_lMax && ver_lMax>0){
					alert(TXT_lunghezzaMinCampo.replace("[1]","  '"+ver_etichetta+"'  ").replace("[2]"," "+ver_lMax+" ").replace("[3]","  "+ver_value.length));
					frm.elements[k].focus();
					return false;
				}
			}
			if (ver_type=="date"){ // se è una data
				if(frm.elements[k].value=='0000-00-00' || frm.elements[k].value==''){
					alert(TXT_inserireValore.replace("[1]"," '"+ver_etichetta+"' "));
					frm.elements[k].focus();
					return false;
				}
			}
			if (ver_type=="select-one"){ // se è un testo
				if(frm.elements[k].selectedIndex==0){
					alert(TXT_selezionareValore.replace("[1]"," '"+ver_etichetta+"' "));
					frm.elements[k].focus();
					return false;
				}
			}
			if (ver_type=="checkbox"){ // se è un testo
				if(!frm.elements[k].checked){
					alert(TXT_selezionareCasella.replace("[1]"," '"+ver_etichetta+"' "));
					frm.elements[k].focus();
					return false;
				}
			}
			...
			if (ver_funzione!=''){
				...
				if(ver_funzione!="@" && ver_funzione!="num" && ver_funzione!="int")ret=eval(ver_funzione);
				if(!ret) return false;
			}
		}
	}
